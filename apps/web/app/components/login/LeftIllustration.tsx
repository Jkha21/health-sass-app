...
export default function LeftIllustration() {
  ...
  return (
    <div className="relative h-[46vh] min-h-[260px] max-h-[350px] w-full shrink-0 overflow-hidden before:absolute before:bottom-0 before:left-0 before:right-0 before:z-[19] before:h-[120px] before:pointer-events-none before:bg-gradient-to-b before:from-transparent before:via-[#0b0705]/50 before:to-[#0b0705] md:h-full md:max-h-none md:min-h-0 md:w-[48%] md:rounded-r-[26px] md:before:content-none">
      
      {/* Structural Injector Style Hook for Leaf Fall Animation (Eliminates config file changes) */}
      <style dangerouslySetInnerHTML={{__html: `
        @keyframes customLeafFall {
          0% { opacity: 0.9; transform: translateY(-10px) rotate(0deg) translateX(0px); }
        ...
        .animate-arbitrary-fallback {
          animation: customLeafFall linear infinite forwards;
        }
      `}} />

      {/* Decorative Landscape SVG Vector Canvas */}
      ...
      {/* Brand Header */}
      <div className="absolute top-[20px] left-1/2 z-20 flex -translate-x-1/2 items-center gap-[9px] whitespace-nowrap rounded-[100px] border border-white/15 bg-[#0b0705]/28 px-[15px] py-[7px] text-[12.5px] font-semibold tracking-[0.12em] uppercase text-white/92 backdrop-blur-md md:top-[26px] md:left-[26px] md:translate-x-0 md:rounded-none md:border-none md:bg-transparent md:p-0 md:backdrop-blur-none">
      ...
      {/* Typography Overlay */}
      <div className="absolute top-1/2 bottom-auto left-0 right-0 z-20 -translate-y-[62%] px-6 text-center text-white md:top-auto md:bottom-12 md:left-[34px] md:right-[34px] md:translate-y-0 md:text-left md:px-0">
        <div className="mb-[5px] text-[11px] font-bold tracking-[0.1em] uppercase opacity-75 md:mb-1.5 md:text-[11.5px] md:tracking-[0.08em] md:opacity-[0.75]">
          Your Digital Record
        </div>
        <h2 className="font-sans text-[clamp(22px,6vw,28px)] font-black leading-[1.14] tracking-tight drop-shadow-[0_2px_24px_rgba(0,0,0,0.4)] md:text-[clamp(28px,3vw,38px)] md:leading-[1.12] md:drop-shadow-[0_2px_20px_rgba(0,0,0,0.25)]">
          Smart Health for<br />Smarter Work
        </h2>
        <button className="hidden md:inline-flex items-center justify-center w-[40px] h-[40px] rounded-full bg-white/18 backdrop-blur-[6px] border border-white/25 cursor-pointer mt-4 text-white transition-all duration-200 hover:bg-white/30 hover:scale-[1.03]" aria-label="Learn more" type="button">
      ...
      <div className="absolute inset-0 z-12 overflow-hidden pointer-events-none">
        {leaves.map((leaf, index) => {
          ...
          return (
            <div
              key={leaf.id}
              className="animate-arbitrary-fallback absolute"
              style={{
                left: `${leaf.left}%`,
                top: "-25px",
                width: `${leaf.size}px`,
